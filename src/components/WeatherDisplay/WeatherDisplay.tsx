  ...
  if (isLoading) {
    return (
      <div role="status" aria-label="loading weather" className="flex flex-col items-center gap-3 p-8">
        ...
        <p className="text-yellow-600 dark:text-yellow-200 font-medium">Fetching weather...</p>
      </div>
    );
  }
  ...
  return (
    <motion.div className="p-6 rounded-2xl border border-gray-200 shadow bg-white/90 dark:bg-yellow-900/70 dark:border-yellow-700 text-center backdrop-blur-md" variants={variants} initial="hidden" animate="visible" aria-label="weather result">
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-lg text-gray-600 dark:text-yellow-200">{weatherLabel}</p>
        <p className="text-4xl font-bold text-yellow-700 dark:text-yellow-200">{Math.round(temperatureC)}°C</p>
        {windSpeed != null && <p className="text-sm text-gray-500 dark:text-yellow-300">Wind: {Math.round(windSpeed)} km/h</p>}
        {fetchedAt && <p className="text-xs text-gray-400 dark:text-yellow-400">As of {new Date(fetchedAt).toLocaleTimeString()}</p>}
      </div>
    </motion.div>
  );
};
